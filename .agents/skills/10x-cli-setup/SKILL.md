...
This skill sets up the `@przeprogramowani/10x-cli` on Linux. The core principle is simple: **the README is the single source of truth**. The CLI evolves — version requirements change, new install methods appear, commands get updated. Rather than hardcoding any of that here, this skill tells you *how to work*, and the README tells you *what to do*.

All commands use Linux bash syntax and paths (`~/.config/10x-cli/`, `$HOME`, etc.).
...
- **Linux-only.** No OS detection needed. Use bash syntax, `~/.config/10x-cli/` paths, and Linux-specific commands.
