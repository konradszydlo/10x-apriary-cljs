...
```bash
cat ~/.config/10x-cli/config.json 2>/dev/null || echo "{}"
```
...
Use the environment context from Step 1 to personalize every answer. All commands use Linux bash syntax and paths.
...
The tool choice is saved in the config file at `~/.config/10x-cli/config.json`. Future `get` commands will use the new tool without needing `--tool` again.
